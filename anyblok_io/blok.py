...
class BlokImporterException(Exception):
    pass
...
class BlokImporter:
    ...
    def import_file(self, Importer, model, *file_path, **kwargs):
        ...
        logger.info(
            "Create %d entries, Update %d entries (%d.%d sec)",
            len(res["created_entries"]),
            len(res["updated_entries"]),
            dt.seconds,
            dt.microseconds,
        )
        if "error_found" in res and res["error_found"]:
            for error in res["error_found"]:  # pragma: no cover
                logger.error(error)
        else:
            importer.delete()
