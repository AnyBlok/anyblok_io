...
class TestImporterFormater:
    ...
    def get_value(self, value, ctype, external_id=False, model=None):
        return self.registry.IO.Importer().str2value(
    ...
    def test_one2many_external_ids_is_none_2(self):
        value = self.get_value(
            dumps([]), "One2Many", external_id=True, model="Model.System.Model"
        )
        assert value == []
    ...
    def test_many2one_external_ids(self):
        ...
        self.registry.IO.Mapping.set(key, model)
        value = self.get_value(
            key, "Many2One", external_id=True, model="Model.System.Model"
        )
        assert value == model

    def test_many2one_external_ids_with_none_value(self):
        value = self.get_value(
            None, "Many2One", external_id=True, model="Model.System.Model"
        )
    ...
    def test_one2one_external_ids(self):
        ...
        self.registry.IO.Mapping.set(key, model)
        value = self.get_value(
            key, "One2One", external_id=True, model="Model.System.Model"
        )
        assert value == model
